...
App.renderHistory = function () {
  ...
  if (!lista.length) {
    App.renderEmptyState(
      container,
      query ? 'Nenhum evento encontrado para essa busca.' : 'Nenhum evento no histórico ainda.'
    );
    return;
  }
...
App._matchesHistoryQuery = function (evento, convidados, equipe, query) {
  const partes = [
    ...
    ...convidados.map((p) => [p?.nome, p?.placa, p?.telefone, p?.email].filter(Boolean).join(' ')),
    ...equipe.map((f) => [f?.nome, f?.cargo, f?.telefone, f?.observacoes].filter(Boolean).join(' ')),
  ];
...
App._renderHistoryCard = function (evento, convidados, equipe) {
  return `
    <div class="history-card">
      ...
      ${App._renderAttendeeSection('Funcionários do evento', equipe, 'equipe', App._renderHistoryEmployeeItem)}
    </div>
  `;
};
...
App._renderAttendeeSection = function (titulo, lista, chipLabel, renderItem) {
  const conteudo = lista.length
    ...
    : `<div class="empty">Nenhum ${chipLabel === 'convidado' ? 'convidado cadastrado' : 'funcionário vinculado'} neste evento.</div>`;
...
App._renderHistoryEmployeeItem = function (funcionario) {
